...
if( ! function_exists( 'docfi_related_team' )){
	
	function docfi_related_team(){
		$thumb_size = 'full';
		$post_id = get_the_id();	
		$number_of_avail_post = '';
		$current_post = array( $post_id );
		$title_length = DocfiTheme::$options['related_team_title_limit'] ? DocfiTheme::$options['related_team_title_limit'] : '';
		$related_post_number = DocfiTheme::$options['related_team_number'];
		...
		# Get related posts by categories ----------
			...
			$terms = get_the_terms( $post_id, 'docfi_team_category' );
			...
			<div class="rt-swiper-slider related-team" data-xld = '<?php echo esc_attr( $swiper_data ); ?>'>
	            <div class="rt-related-title">
					<div class="title-holder">
						<h3 class="entry-title has-animation"><?php echo wp_kses( DocfiTheme::$options['team_related_title'] , 'alltext_allow' ); ?><span class="titleline"></span></h3>
					</div>
									...
									<a href="<?php the_permalink();?>">
										...
										if ( has_post_thumbnail() ){
											...
											if ( !empty( DocfiTheme::$options['no_preview_image']['id'] ) ) {
												...
												echo '<img class="wp-post-image" src="' . DocfiTheme_Helper::get_img( 'noimage_400X400.jpg' ) . '" alt="'.get_the_title().'">';
											}
										}
										?>
									</a>
