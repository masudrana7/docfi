...
class WC_Functions {
	...
	public function __construct() {
		...
		add_action( 'loop_shop_per_page',                  array( $this, 'docfi_loop_shop_per_page' ), 20 );
		add_action( 'woocommerce_after_shop_loop',         array( $this, 'docfi_products_paginations' ), 10 );
		// Columns
		...
		/* ====== Checkout Page ====== */
		add_filter( 'woocommerce_checkout_fields', array( $this, 'docfi_checkout_fields' ) );

		/* ====== Mini Cart ====== */
		add_action( 'docfi_woo_cart_icon', array( $this, 'docfi_wc_cart_count' ) );
		add_action( 'woocommerce_add_to_cart_fragments', array( $this, 'docfi_header_add_to_cart_fragment' ) );
		add_action( 'wp_ajax_docfi_product_remove', array( $this, 'docfi_product_remove' ) );
        add_action( 'wp_ajax_nopriv_docfi_product_remove', array( $this, 'docfi_product_remove' ) );
	}
	...
	public function loop_shop_columns(){
		if( isset($_GET["shopview"]) && $_GET["shopview"] == 'list' ) {
			...
			$cols = DocfiTheme::$options['products_cols_width'];
		}
		return $cols;
	}

	public function docfi_products_paginations(){
		DocfiTheme_Helper::pagination();
	}
	...
	public function single_product_layout_hooks() {
		...
		add_action( 'woocommerce_single_product_summary', function(){
			global $product;
			$module_data = [
				'wc_shop_wishlist_icon' => DocfiTheme::$options['wc_product_wishlist_icon'],
				'wc_shop_compare_icon' => DocfiTheme::$options['wc_product_compare_icon'],
			];?>
	...
	public static function print_add_to_cart_icon( $product_id, $icon = true, $text = true ){
		...
		if ( $in_cart ) {
			...
			if ( $icon ) {
				$html .= '<i class="icon-docfi-cart"></i>';
			}
			if ( $text ) {
    ...
    public function docfi_loop_shop_per_page( $products ) {
        $shop_posts_per_page = DocfiTheme::$options['products_per_page'];
	...
	public static function docfi_product_social_share(){
		...
		<div class="post-share-btn">
			<h5 class="item-label"><?php esc_html_e( 'Share:', 'docfi' );?></h5>
	...
	public static function get_stock_status() {
		global $product;
		return $product->is_in_stock() ? esc_html__( 'In Stock', 'docfi' ) : esc_html__( 'Out of Stock', 'docfi' );
	}

	public function product_review_form( $comment_form ){
		$commenter = wp_get_current_commenter();
		$comment_form['fields'] = array(
			'author' => '<div class="row"><div class="col-sm-6"><div class="comment-form-author form-group"><input id="author" name="author" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" placeholder="' . esc_attr__( 'Name *', 'docfi' ) . '" required /></div></div>',
			'email'  => '<div class="comment-form-email col-sm-6"><div class="form-group"><input id="email" class="form-control" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" placeholder="' . esc_attr__( 'Email *', 'docfi' ) . '" required /></div></div></div>',
		);

		$comment_form['comment_field'] = '';

		if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
			$comment_form['comment_field'] = '<p class="comment-form-rating"><label for="rating">' . esc_html__( 'Your Rating', 'docfi' ) .'</label>
			<select name="rating" id="rating" required>
			<option value="">' . esc_html__( 'Rate&hellip;', 'docfi' ) . '</option>
			<option value="5">' . esc_html__( 'Perfect', 'docfi' ) . '</option>
			<option value="4">' . esc_html__( 'Good', 'docfi' ) . '</option>
			<option value="3">' . esc_html__( 'Average', 'docfi' ) . '</option>
			<option value="2">' . esc_html__( 'Not that bad', 'docfi' ) . '</option>
			<option value="1">' . esc_html__( 'Very Poor', 'docfi' ) . '</option>
			</select></p>';
		}

		$comment_form['comment_field'] .= '<div class="form-group comment-form-comment"><textarea id="comment" name="comment" class="form-control" placeholder="' . esc_attr__( 'Your Review *', 'docfi' ) . '" cols="45" rows="8" required></textarea></div>';

		return $comment_form;
	}

	// WooCommerce Checkout Fields Hook
    public function docfi_checkout_fields( $fields ) {
    ...
    public static function DocfiWooMiniCart(){
        ob_start();
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            do_action( 'docfi_woo_cart_icon' );
        }
    ...
    public function docfi_cart_items(){
        $empty_cart = '<li class="cart-item d-flex align-items-center"><h5 class="text-center no-cart-items">'. apply_filters( 'docfi_woo_mini_cart_empty', esc_html__('Your cart is empty', 'docfi') ) .'</h5></li>';
        if(is_null(WC()->cart)) {
        	return $empty_cart;
		}
        ...
        $remove_loader = apply_filters('woo_mini_cart_loader', DocfiTheme_Helper::get_img( 'spinner2.gif' ));
                ...
                <li class="cart-item d-flex align-items-center">
	                ...
	                <div class="cart-single-product">
						<div class="media">
										...
										<?php
				                            echo apply_filters(
				                                ...
				                                sprintf(
				                                    ...
				                                    esc_attr__( 'Remove this item', 'docfi' ),
				                                    esc_attr( $product_id ),
							...
							<span class="remove-item-overlay text-center"><img src="<?php echo esc_url($remove_loader); ?>" alt="<?php esc_attr_e('Loader..', 'docfi') ?>" /></span>
						</div>
					</div>
            ...
            <li class="cart-total">
                <div class="total-price">
                    <span class="f-left"><?php _e( 'Total:', 'docfi' ); ?></span>
                    <span class="f-right"><?php echo WC()->cart->get_cart_subtotal(); ?></span>
                </div>
            </li>
            <li class="cart-btn">
                <div class="checkout-link">
                    <a class="button wc-forward" href="<?php echo wc_get_cart_url(); ?>"><?php _e( 'View Cart', 'docfi' ); ?></a>
                    <a class="button wc-forward checkout" href="<?php echo wc_get_checkout_url(); ?>"><?php _e( 'Checkout', 'docfi' ); ?></a>
                </div>
            </li>
            <?php endif; ?>
        <?php 
    ...
    public function docfi_wc_cart_count() {     
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
   			if(is_null(WC()->cart)) {
            ...
            <div class="cart-list-trigger">
	            <a class="cart-contents cart-trigger-icon" href="<?php echo esc_url( $cart_link ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'docfi' ); ?>"><i class="icon-docfi-cart"></i> <?php if ( $count > 0 ) echo '(' . $count . '2' . ')'; ?></a>
                <div class="cart-wrapper">
                    <ul class="minicart">
		            	<?php echo wp_kses_stripslashes( $this->docfi_cart_items() ); ?>
		            </ul>
                </div>
            </div>
            <?php
    ...
    public function docfi_header_add_to_cart_fragment( $fragments ) {
        ob_start();
        if(!is_null(WC()->cart)) {
		    $count = WC()->cart->cart_contents_count;
		} else {
        ...
        ?>
            <div class="header-action-item cart-area mini-cart-items header-shop-cart">
            	<div class="cart-list-trigger">
	                <a class="cart-contents cart-trigger-icon" href="<?php echo esc_url( $cart_link ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'docfi' ); ?>"><i class="icon-docfi-cart"></i> <?php if ( $count >= 0 ) echo '<span>' . $count . '</span>'; ?></a>
	                <div class="cart-wrapper">
		                <ul class="minicart">
		                <?php echo wp_kses_stripslashes( $this->docfi_cart_items() ); ?>
		                </ul>
    ...
    public function docfi_wc_cart_ajax() {
        $output = '';
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
        	if(!is_null(WC()->cart)) {
			    $count = WC()->cart->cart_contents_count;
            ...
            ob_start();
            ?>
            <a class="cart-contents" href="<?php echo esc_url( $cart_link ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'docfi' ); ?>"><i class="icon-docfi-cart"></i> <?php if ( $count > 0 ) echo '(' . $count . ')'; ?></a>
            <ul class="minicart">
            <?php
                echo wp_kses_stripslashes( $this->docfi_cart_items() );
            ?>
            </ul>
            <?php
            $output = ob_get_clean();
    ...
    public function docfi_product_remove() {
        ...
        $return["mini_cart"] = $this->docfi_wc_cart_ajax();
    ...
    public function single_meta_actions_button( $product, $module_data ) {
        ...
        $product_id = $product->get_id();
        if( isset( $module_data['wc_shop_wishlist_icon'] ) && $module_data['wc_shop_wishlist_icon'] ){ ?>
        	<span class="wishlist"><?php do_action( 'rtsb/modules/wishlist/print_button', $product_id );echo esc_html("Add To Wishlist", 'docfi'); ?></span>
		<?php }
        if( isset( $module_data['wc_shop_compare_icon'] ) && $module_data['wc_shop_compare_icon'] ){ ?>
        	<span class="compare"><?php do_action( 'rtsb/modules/compare/print_button', $product_id );echo esc_html("Add To Compare", 'docfi'); ?></span>
        <?php }
